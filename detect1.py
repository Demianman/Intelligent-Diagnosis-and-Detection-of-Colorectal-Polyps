...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        hsv_img = hsv_img / 255.0  # Normalize to 0.0 - 1.0

        # Calculate dx and dy
        dx = cv2.Sobel(img, cv2.CV_64F, 1, 0, ksize=3)
        dy = cv2.Sobel(img, cv2.CV_64F, 0, 1, ksize=3)
        dx = dx / 255.0  # Normalize to 0.0 - 1.0
        dy = dy / 255.0  # Normalize to 0.0 - 1.0

        # Concatenate the channels
        expanded_img = np.concatenate((img, hsv_img, dx, dy), axis=1)
        img = torch.from_numpy(expanded_img).to(device)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    x0 = format(x0/480, '.6f')  #此处我要保存和训练标注的txt一样的格式，故保留六位小数
                    y0 = format(y0/480, '.6f')
                    chang = format(chang/480, '.6f') # 480和480为我的图片长和宽
                    kuan = format(kuan/480, '.6f')
