...
#---------------------------------
def rgb_hsv_grad_fusion(img, save_debug=False, debug_dir="debug_output", visualize_fusion=False):
    # 确保输入是标准三通道
    if img.shape[2] != 3:
        raise ValueError(f"Input image must have 3 channels, but got {img.shape[2]} channels")
    ...
    rgb = img
    if save_debug:
        cv2.imwrite(os.path.join(debug_dir, "step1_rgb.png"), (rgb * 255).astype(np.uint8))
    ...
    grad = np.clip(grad / grad.max() * 255.0, 0, 255).astype(np.uint8)  # (H, W, 3)


    if save_debug:
        cv2.imwrite(os.path.join(debug_dir, "step4_gradx.png"), (grad_x * 255).astype(np.uint8))
        cv2.imwrite(os.path.join(debug_dir, "step5_grady.png"), (grad_y * 255).astype(np.uint8))

        # 直接堆叠
    out = np.concatenate([img, hsv, grad], axis=2)  # (H, W, 9)

    # 7. 伪彩色可视化（可选）
    if visualize_fusion:
        # 简单方法：取8通道中前3个合成一个伪彩图
        pseudo_vis = out[..., :3]  # 直接拿第1、2、3通道
        pseudo_vis = np.clip(pseudo_vis * 255, 0, 255).astype(np.uint8)
        cv2.imwrite(os.path.join(debug_dir, "fusion_pseudocolor.png"), pseudo_vis)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        img = rgb_hsv_grad_fusion(img, save_debug=True, debug_dir="debug_output", visualize_fusion=True)
